...
const STATUS_COLORS: Record<string, string> = {
  bleed: "#ff4444", poison: "#ff4444",
  slow: "#ff8800", silence: "#ff8800",
  immobilize: "#aa44ff", taunt: "#aa44ff", frozen: "#aa44ff",
  wet: "#88ccff", burn: "#ff8800",
};
...
interface CharacterInfo {
  data: { position: { x: number; y: number }; current_hp: number; max_hp: number };
  status: string;
}
...
export class BattleHud {
  ...
  createHpBar(entityId: string, worldX: number, worldY: number, currentHp: number, maxHp: number) {
    const barY = worldY + HP_BAR_OFFSET_Y;
    const bg = this.scene.add.rectangle(worldX, barY, HP_BAR_WIDTH, HP_BAR_HEIGHT, 0x333333);
    bg.setOrigin(0.5);
    bg.setDepth(100);
    ...
    const fill = this.scene.add.rectangle(fillX, barY, fillWidth, HP_BAR_HEIGHT, hpColor(pct));
    fill.setOrigin(0.5);
    fill.setDepth(100);
  ...
  updateHpBar(entityId: string, worldX: number, worldY: number, currentHp: number, maxHp: number) {
  ...
  updateStatusIcons(entityId: string, worldX: number, worldY: number, effects: Set<string>) {
    ...
    const label = this.scene.add.text(worldX, worldY + HP_BAR_OFFSET_Y - 14, abbrs.join(" "), {
      fontSize: "10px",
      color,
  ...
  spawnFloatingText(worldX: number, worldY: number, text: string, color: string, fontSize: string) {
    this.floatingOffset += 12;
    const textObj = this.scene.add.text(worldX, worldY - 20 - this.floatingOffset, text, {
      fontSize,
      color,
      fontFamily: "monospace",
      fontStyle: "bold",
  ...
  updateAllBars(
    ...
    for (const [entityId, entry] of characters) {
      ...
      const { px, py } = gridToPixel(entry.data.position.x, entry.data.position.y);
      this.updateHpBar(entityId, px, py, entry.data.current_hp, entry.data.max_hp);
    }
  }
