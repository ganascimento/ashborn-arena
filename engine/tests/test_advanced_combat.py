...
from engine.models.ability import ABILITIES
...
class TestAoEExpansion:
    def test_aoe_hits_multiple_enemies(self):
        ...
        bs.execute_action(
            ACTION_ABILITY_2, target_idx
        )  # ABILITY_2 = slot 1 = nova_flamejante

        hit_count = sum(
            1 for eid in enemies if bs.get_character(eid).current_hp < hp_before[eid]
        )
    ...
    def test_aoe_friendly_fire(self):
        ...
        assert ally_hp_after < ally_hp_before, (
            "AoE should deal friendly fire damage to ally"
        )
    ...
    def test_aoe_no_evasion(self):
        ...
        assert bs.get_character(archer_id).current_hp < hp_before, (
            "AoE should always hit (no evasion)"
        )
...
class TestChainDamage:
    ...
    def test_chain_secondary_damage_is_70_percent(self):
        ...
        if primary_dmg > 0 and secondary_dmg > 0:
            ...
            assert 0.5 <= ratio <= 0.9, (
                f"Chain should be ~70% of primary, got {ratio:.2f}"
            )
    ...
    def test_chain_only_enemies(self):
        ...
        assert bs.get_character(cleric_id).current_hp == ally_hp_before, (
            "Chain should not hit allies"
        )
...
class TestDamageReflect:
    def test_reflect_damages_attacker(self):
        ...
        em.apply_effect(
            cleric_id, Effect("reflect", EffectType.BUFF, cleric_id, 2, 0.30)
        )
        ...
        assert attacker_hp_after < attacker_hp_before, (
            "Reflect should damage the attacker"
        )
...
class TestDamageRedirect:
    def test_redirect_splits_damage(self):
        ...
        em = bs.get_effect_manager()
        em.apply_effect(
            mage_id, Effect("redirect", EffectType.BUFF, cleric_id, 2, 0.40)
        )
    ...
    def test_redirect_does_not_work_if_caster_dead(self):
        ...
        bs.get_character(cleric_id).apply_damage(
            bs.get_character(cleric_id).max_hp + 11
        )
...
class TestUntargetable:
    def test_untargetable_excluded_from_masking(self):
        ...
        em = bs.get_effect_manager()
        em.apply_effect(
            assassin_id, Effect("untargetable", EffectType.BUFF, assassin_id, 1)
        )
        ...
        target_idx = (
            bs.get_position(assassin_id).y * 10 + bs.get_position(assassin_id).x
        )
    ...
    def test_aoe_skips_untargetable(self):
        ...
        em = bs.get_effect_manager()
        em.apply_effect(
            enemies[1], Effect("untargetable", EffectType.BUFF, enemies[1], 1)
        )
...
class TestPoisonOnHit:
    def test_poison_attacks_applies_dot(self):
        ...
        em = bs.get_effect_manager()
        em.apply_effect(
            assassin_id, Effect("poison_attacks", EffectType.BUFF, assassin_id, 3, 3.0)
        )
    ...
    def test_poison_attacks_consumes_charges(self):
        ...
        em = bs.get_effect_manager()
        em.apply_effect(
            assassin_id, Effect("poison_attacks", EffectType.BUFF, assassin_id, 1, 3.0)
        )
...
class TestNextAttackBonus:
    def test_olho_do_predador_doubles_damage(self):
        ...
        em = bs.get_effect_manager()
        em.apply_effect(
            archer_id, Effect("next_attack_bonus", EffectType.BUFF, archer_id, 2, 1.0)
        )
    ...
    def test_bonus_consumed_after_one_attack(self):
        ...
        em = bs.get_effect_manager()
        em.apply_effect(
            archer_id, Effect("next_attack_bonus", EffectType.BUFF, archer_id, 2, 0.5)
        )
...
class TestRangeBonus:
    def test_range_bonus_extends_attack_range(self):
        ...
        em = bs.get_effect_manager()
        em.apply_effect(
            archer_id, Effect("range_bonus", EffectType.BUFF, archer_id, 2, 2.0)
        )
